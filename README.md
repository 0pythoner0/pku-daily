...
- **已内置官网源（自动抓取）**：教务部通知公告、学工部资讯通知、学工部心理健康教育与咨询中心、北大新闻网综合新闻、北大新闻网信息预告（讲座/活动预告）、计算中心通知、工学院本科生通知、工学院学院公告、工学院通知发布（本科生学院·奖学金/评优）、工学院讲座信息、总务部后勤动态、总务部相关通知、力学与工程科学学院通知、力学与工程科学学院学术活动（讲座）、力学与工程科学学院通知发布（学生事务）。
- **近两天窗口**：`sources.json` 里的 `days_lookback = 2`，只推送 **近 2 天** 发布的内容（没有日期的页面保守保留）。不做"跨天历史去重"、也不限制条数——所有符合"近 2 天 + 大一相关"的文章都会推送；但默认在**同一次运行内按标题去重**（`dedup_by_title = true`），一条通知若挂在多个栏目只列一次，不会重复刷屏。
